...
(function($) {
  ...
  function evfun(fun, hint) {
    // We have to allow for both raw "function.." and "(function.."
    // since CoffeeScript generates the latter form.
    // In fact, we allow for the keyword "function" to be preceeded by any
    // number of left parentheses or whitespaces
    if (fun && fun.match && fun.match(/^(\s|\()*function/)) {
      eval("var f = "+fun);
  ...
  $.evently = {
    // The 'connect' function expects either a list of events
    // or a hash from source to target events; in the latter
    // case, a different event will be used on the target
    connect : function(source, target, events) {
      $.each(events, function(key, ev) {
        // TODO: to check this every single iteration is a bit silly
        var sourceEv = typeof(key) == "number" ? ev : key;
        $(source).bind(sourceEv, function() {
